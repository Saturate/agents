#!/bin/bash
#
# Instructions loaded logger for Claude Code hooks.
# Handles InstructionsLoaded events.
# Writes to ~/.claude/logs/configs-YYYY-MM-DD.jsonl
#
# Never blocks — always exits 0.

source "$(dirname "$0")/lib.sh"

[ "$HOOK_EVENT" = "InstructionsLoaded" ] || exit 0

TIMESTAMP=$(utc_timestamp)

ENTRY=$(jq -cn \
  --arg ts "$TIMESTAMP" \
  --arg ev "instructions_loaded" \
  --arg sid "$SESSION_ID" \
  --arg project "$PROJECT" \
...
emit_event "configs" "$ENTRY" "$(jq -cn \
  --arg source "claude-code" \
  --arg event "instructions_loaded" \
  --arg project "$PROJECT" \
